...
class Schrodinger:
    ...
    @staticmethod
    def D2mat_dirichlet(numpts, delta=1):
        ...
        n_int = numpts - 2
        a = 1.0 / delta**2 * np.ones(n_int)
        b = -2.0 / delta**2 * np.ones(n_int)
        c = 1.0 / delta**2 * np.ones(n_int)
        return sparse.spdiags([a, b, c], [-1, 0, 1], n_int, n_int)


    def Hamiltonian(self):
        """Constructs Hamiltonian using the potential and kinetic energy terms.

        For non-periodic axes, we enforce Dirichlet BC (psi=0 on boundaries) by
        solving only on interior points and padding eigenfunctions with zeros.
        """
        dx = self.x[1] - self.x[0]
        dy = self.y[1] - self.y[0]

        # Choose operators per-axis
        D2x = Schrodinger.D2mat_dirichlet(len(self.x), dx)
        nx_i = len(self.x) - 2
        D2y = Schrodinger.D2mat_dirichlet(len(self.y), dy)
        ny_i = len(self.y) - 2

        # Slice potential to interior where needed (U is expected shaped (ny, nx))
        U2 = self.U[1:-1, 1:-1]
    ...
    def get_2Dpsis(self, num_levels=-1):
        ...
        If Dirichlet BC are used (non-periodic axis), eigenvectors are solved on
        the interior grid and padded with zeros on the boundaries.
        """
        nx = len(self.x)
        ny = len(self.y)
        ...
        for psi in self.psis(num_levels):
            psi2 = np.reshape(psi, (ny_i, nx_i))
            full = np.zeros((ny, nx), dtype=psi2.dtype)
            full[ys, xs] = psi2
            psis_full.append(full)
        return psis_full
...
class QuantumAnalysis():
    ...
    def solve_system(
        ...
        self.dy = float(self.ysol[1] - self.ysol[0])
    ...
    def get_quantum_spectrum(self, coor: Optional[List[float]] = [0, 0], dxdy: List[float] = [1, 2], plot_wavefunctions: bool = False,
        ...
        for k in range(6):
            ...
            if plot_wavefunctions:
                zdata = -self.potential
                contours = [np.round(np.min(zdata), 3) +
                            k*1e-3 for k in range(5)]
                CS = plt.contour(
                    self.x, self.y, zdata, levels=contours)
    ...
    def get_resonator_coupling(self, coor: Optional[List[float]] = [0, 0], dxdy: List[float] = [1, 2], Ex: float = 0, Ey: float = 1e6, resonator_impedance: float = 50,
        ...
        if hasattr(self, "Xsol") and hasattr(self, "Ysol"):
            X, Y = self.Xsol, self.Ysol
            dA = getattr(self, "dA", (self.xsol[1] - self.xsol[0]) * (self.ysol[1] - self.ysol[0]))
        else:
            X, Y = np.meshgrid(self.xsol, self.ysol)
            dA = (self.xsol[1] - self.xsol[0]) * (self.ysol[1] - self.ysol[0])

        prefactor = q_e * np.sqrt(hbar * (2 * np.pi * resonator_frequency) ** 2 * resonator_impedance / 2) * 1 / (2 * np.pi * hbar)

        # Flatten and vectorize: g = prefactor * ∬ ψ_i*(r) (E·r) ψ_j(r) dA
        dip = (X * Ex + Y * Ey).reshape(-1)  # (M,)
        Psi = np.stack([psi.reshape(-1) for psi in self.Psis], axis=0)  # (N, M)
        g_ij = prefactor * dA * (Psi * dip) @ Psi.conj().T  # (N, N)
